...
class DatabaseTools:
    ...
    def __init__(self):
        self.herramientas_disponibles = {
            ...
            'historial_citas_paciente': self.historial_citas_paciente, # <-- Nueva herramienta
        }
    ...
    def historial_citas_paciente(self, paciente_id):
        """Obtiene el historial de citas de un paciente específico."""
        try:
            if not paciente_id:
                return {"error": "Se requiere el ID del paciente."}

            citas = CitaMedica.objects.filter(
                paciente_id=paciente_id
            ).select_related('doctor', 'doctor__especialidad').values(
                'fecha',
                'hora_cita',
                'estado',
                'doctor__nombres',
                'doctor__apellidos',
                'doctor__especialidad__nombre',
                'observaciones'
            ).order_by('-fecha', '-hora_cita')
            ...
            if not citas.exists():
                return {
                    ...
                    "total_citas": 0,
                    "citas": [],
                    "mensaje": f"El paciente con ID {paciente_id} no tiene citas registradas."
                }
            ...
            return {
                ...
                "total_citas": citas.count(),
                "citas": list(citas),
                "consulta": f"historial de citas del paciente ID {paciente_id}"
            }

        except Exception as e:
            logger.error(f"Error en historial_citas_paciente: {e}")
            return {"error": str(e)}
